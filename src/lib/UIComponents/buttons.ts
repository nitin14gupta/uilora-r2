import { getCode } from "../getCode";
import { Component } from "../types";

export const buttonComponents: Component[] = [
    {
        id: "uilora-modern-buttons",
        name: "Uilora Modern Buttons",
        description: "A collection of modern SaaS-style buttons including Border Beam, Spotlight, Shimmer, and Cosmic Glow effects.",
        category: "UI Components",
        installType: "cli",
        tags: ["button", "modern", "saas", "glow", "spotlight"],
        previewUrl: "/UIComponents/Button/Modern",
        code: {
            tsx: getCode("src/components/UIComponents/Button/tsx/UiloraModernButtons.tsx"),
            jsx: getCode("src/components/UIComponents/Button/jsx/UiloraModernButtons.jsx"),
        },
        usageCode: {
            tsx: getCode("src/app/(routes)/UIComponents/Button/page.tsx")
        },
        dependencies: ["lucide-react", "framer-motion", "clsx", "tailwind-merge"],
    },
    {
        id: "uilora-action-buttons",
        name: "Uilora Action Buttons",
        description: "Specialized action buttons featuring Rocket launches, Printer effects, Shatter text, and Confetti celebrations.",
        category: "UI Components",
        installType: "cli",
        tags: ["button", "action", "rocket", "printer", "confetti"],
        previewUrl: "/UIComponents/Button/Action",
        code: {
            tsx: getCode("src/components/UIComponents/Button/tsx/UiloraActionButtons.tsx"),
            jsx: getCode("src/components/UIComponents/Button/jsx/UiloraActionButtons.jsx"),
        },
        usageCode: {
            tsx: getCode("src/app/(routes)/UIComponents/Button/page.tsx")
        },
        dependencies: ["lucide-react", "framer-motion"],
    },
    {
        id: "uilora-interactive-buttons",
        name: "Uilora Interactive Buttons",
        description: "Playful and interactive buttons including Paper Plane, Watchful Eyes, and the Runaway button.",
        category: "UI Components",
        installType: "cli",
        tags: ["button", "interactive", "fun", "eye-tracking", "send"],
        previewUrl: "/UIComponents/Button/Interactive",
        code: {
            tsx: getCode("src/components/UIComponents/Button/tsx/UiloraInteractiveButtons.tsx"),
            jsx: getCode("src/components/UIComponents/Button/jsx/UiloraInteractiveButtons.jsx"),
        },
        usageCode: {
            tsx: getCode("src/app/(routes)/UIComponents/Button/page.tsx")
        },
        dependencies: ["lucide-react", "framer-motion"],
    },
    {
        id: "uilora-utility-buttons",
        name: "Uilora Utility Buttons",
        description: "Practical utility buttons like the Command Copy button and the Neon Underglow status button.",
        category: "UI Components",
        installType: "cli",
        tags: ["button", "utility", "copy", "neon", "status"],
        previewUrl: "/UIComponents/Button/Utility",
        code: {
            tsx: getCode("src/components/UIComponents/Button/tsx/UiloraUtilityButtons.tsx"),
            jsx: getCode("src/components/UIComponents/Button/jsx/UiloraUtilityButtons.jsx"),
        },
        usageCode: {
            tsx: getCode("src/app/(routes)/UIComponents/Button/page.tsx")
        },
        dependencies: ["lucide-react"],
    }
];
